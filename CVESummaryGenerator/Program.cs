...
namespace CVESummaryGenerator
{
    class MainClass
    {
        ...
        {
            string targetCVEs;
            string pathToTargetCVEs = Path.Combine(Directory.GetCurrentDirectory(), "targetCVEs.txt");
            using (StreamReader sr = new StreamReader(pathToTargetCVEs, Encoding.GetEncoding("Shift_JIS")))
            {
                targetCVEs = sr.ReadToEnd();
            }
